...
from .. import config
from .. import get_simpsons_diversity_index
...
def get_temp_gdb() -> Path:
    ...
    gdb = arcpy.management.CreateFileGDB(
        out_folder_path=gettempdir(),
        out_name='tmpfgdb.gdb'
    )[0]
...
def add_preconfigured_simpsons_diversity_index_feature_class(
    ...
    if index_name not in idx_names:
        raise ValueError(f'Unfortunately, your requested index, "{index_name}," is not available. Please select '
                         f'from {idx_names}')
    ...
    add_simpsons_diversity_index_feature_class(enrich_fc, enrich_fld_lst, simpson_field_name)
